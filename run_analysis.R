...
#2-Extracts only the measurements on the mean and standard deviations for each measurement.
        ...
        DataSet<-DataSet[ ,sort(c(1,2,intersect(sort(c(grep("-mean\\()-",names(DataSet)), grep("-std\\()-",names(DataSet)))),grep("^f",names(DataSet)))))]        ##DataSet<-DataSet[ ,sort(c(1,2,grep("mean\\()",names(DataSet)), grep("std\\()",names(DataSet))))]
        DataSet<-select(DataSet,-grep("Jerk",names(DataSet)))
...
#with the average of each variable for each activity and each subject.
        ...
        write.table(AG,file = "./TidyDataSet.txt")
